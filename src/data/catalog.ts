  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        unit: 'once',
        human: {
          es: 'Consigues un backend en Node.js + PostgreSQL + Redis, con transacciones bien bloqueadas y sin locks sobre las tablas calientes — el mismo patrón que uso ahora mismo en un MVP de transporte en producción, no una arquitectura de manual.',
          ru: 'Вы получаете бэкенд на Node.js + PostgreSQL + Redis, с корректно заблокированными транзакциями и без локов на «горячих» таблицах — тот же паттерн, что я использую прямо сейчас в MVP приложения такси, а не архитектура из учебника.',
          en: "You get a backend on Node.js + PostgreSQL + Redis, with transactions properly locked and no locking on hot tables — the same pattern I'm using right now in a live transport MVP, not a textbook architecture.",
          ka: 'იღებთ ბექენდს Node.js + PostgreSQL + Redis-ზე, სწორად დაბლოკილი ტრანზაქციებით და "ცხელ" ცხრილებზე ლოკების გარეშე — იგივე მიდგომა, რომელსაც ახლა ვიყენებ ტრანსპორტის MVP-ში, არა სახელმძღვანელოს არქიტექტურა.',
        },
      ...
      },
      {
        ...
        unit: 'hour',
        human: {
          es: 'Compras un bloque de horas (mínimo diez) que no caducan nunca, y se descuentan por el tiempo real que dedico a cada petición, no por tarea completa redondeada al alza — así sabes exactamente en qué se gasta cada hora.',
          ru: 'Вы покупаете блок часов (минимум десять), которые никогда не сгорают, и списываются по фактически потраченному времени на каждую задачу, а не округляются в большую сторону — вы точно знаете, на что уходит каждый час.',
          en: "You buy a block of hours (ten minimum) that never expire, deducted by the actual time spent on each request, not rounded up per task — so you know exactly where every hour goes.",
          ka: 'ყიდულობთ საათების პაკეტს (მინიმუმ ათი), რომელიც არასდროს იწურება და იჭრება თითოეულ მოთხოვნაზე დახარჯული რეალური დროის მიხედვით, არა დამრგვალებული ამოცანით — ზუსტად იცით, სად იხარჯება ყოველი საათი.',
        },
...
export const bundles: Bundle[] = [
  ...
  },
  {
    ...
    human: {
      es: 'Consigues backend, reservas en tiempo real y panel de rutas — el punto de partida serio para tu propia app de transporte, no un prototipo de juguete.',
      ru: 'Вы получаете бэкенд, бронирования в реальном времени и панель маршрутов — серьёзную стартовую точку, а не игрушечный прототип.',
      en: 'You get a backend, real-time bookings and a route panel — a serious starting point for your own transport app, not a toy prototype.',
      ka: 'იღებთ ბექენდს, რეალურ დროში ჯავშნებს და მარშრუტების პანელს — სერიოზულ საწყისს, არა სათამაშო პროტოტიპს.',
    },
